...
def heterocedasticidad(Residuos,MVX,nivel_confianza):
  ...
  st.write(f"**LM** = {bp_test[0]}, **p-value** = {round(bp_test[1],4)}")
  ...
  st.write(f"**Contraste de hipótesis**: El p-value asociado al LM es {round(pvalor_bptest,4)}, {inter_bp}")
...
#Pruebas de Multicolinealidad
  ...
  if maximo_VIF < 7 and promedio_VIF < 7:
  ...
  elif maximo_VIF > 7 or promedio_VIF > 7:
