...
Te invito a probar la app. ¡Sigue las instrucciones y empieza a modelar!
...
7. **Asimetría**: Una medida de la falta de simetría (¿que tan cargada esta a la derecha o izquierda?) en la distribución de una variable aleatoria.

8. **Curtosis**: Una medida del "apuntalamiento" o "aplanamiento" de la distribución de una variable aleatoria en comparación con la distribución normal. Una curtosis mayor a cero, indica una distribución leptocurtica; una curtosis de cero, indica una distribución mesocurtica; y una curtosis menor a cero, indica una distribución platocurtica.
...
""", unsafe_allow_html=True)
    ...
    if st.button("Realiza el test de Breusch-Godfrey"):
         ...
         if pvalue_bg >= ns:
              contraste_bg = f"El p-value asociado al LM es {round(pvalue_bg,4)}, mayor o igual al nivel de significancia de {round(ns,2)}, por lo tanto; **No existe evidencia suficiente para rechazar la H0, no existe autocorrelación serial de orden {rezagos}**"
              p_autocorrelacion = True
         elif pvalue_bg < ns:
              contraste_bg = f"El p-value asociado al LM es {round(pvalue_bg,4)}, menor al nivel de significancia de {round(ns,2)}, por lo tanto; **Existe evidencia sufiente para rechazar la H0; existe autocorrelación serial de orden {rezagos}**"
              p_autocorrelacion = False
                 
         st.write("-------------------------------------------")
         st.write("***Prueba de Autocorrelación: Breusch-Godfrey***")
         st.write(f"LM = {round(estadistico_bg,4)}, p-value asociado = {round(pvalue_bg,4)}")
         st.write("""
...
<div class="justifyText">
    ...
    for i in independiente_X:
         clave = i  
         b[clave] = st.number_input(f"Escribe el valor de {i}")  
